 ...
 * DspTester for FFT
 */
class FFTTester[T <: chisel3.Data](c: FFT[T], inp: Seq[Complex], out: Seq[Complex], tolLSBs: Int = 5) extends DspTester(c) with HasTesterUtil[FFT[T]] {
  ...
  for ((value, index) <- inp.zipWithIndex) {
    poke(c.io.in.valid, 1)
    poke(c.io.in.bits, value)
  ...
  fixTolLSBs.withValue(tolLSBs) { expect_complex_seq(c.io.out.bits, out) }
}
...
class IFFTTester[T <: chisel3.Data](c: IFFT[T], inp: Seq[Complex], out: Seq[Complex], pktStart: Boolean = true, pktEnd: Boolean = true, tolLSBs: Int = 2) extends DspTester(c) with HasTesterUtil[IFFT[T]] {
  ...
  poke_complex_seq(c.io.in.bits, inp)
  ...
  out.zipWithIndex.foreach { case (value, index) =>
    ...
    fixTolLSBs.withValue(tolLSBs) { expect(c.io.out.bits, value) }
    step(1)
  }
}
...
object FixedFFTTester {
  def apply(params: FFTParams[FixedPoint], inp: Seq[Complex], out: Seq[Complex]): Boolean = {
