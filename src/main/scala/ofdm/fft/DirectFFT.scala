...
class DirectStageIO[T <: Data : Ring](params: FFTParams[T]) extends Bundle {
  val in  = Input(Vec(params.numPoints, params.protoIQ))
  val out = Output(Vec(params.numPoints, params.protoIQ))
...
trait HasDirectStageIO[T <: Data] {
  val io: DirectStageIO[T]
}
 ...
 */
class CooleyTukeyStage[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module with HasDirectStageIO[T] {
  ...
  if (params.numPoints == base) { // Base case --> use butterfly
    ...
    // Create sub-FFTs
    val new_params = params.copy(numPoints = numPointsDivBase)
 ...
 */
class PFAStage[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module with HasDirectStageIO[T] {
  ...
  val first_stage_params = params.copy(numPoints = N1)
  val rest_stage_params  = params.copy(numPoints = N2)
