...
class SDFFFT[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  val io = IO(SISOIO(params.protoIQ))
...
class SDFFFTDeserIn[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T], val dit: Boolean = true) extends Module {
  ...
  val updated_params = if (params.decimType == OptDecimType) params.copy(decimType = DITDecimType) else params
  val serdes_params  = PacketSerDesParams(params.protoIQ.cloneType, params.numPoints)
  ...
  val out_if = updated_params.decimType match {
    case DITDecimType =>
...
class SDFFFTDeserOut[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  val updated_params = if (params.decimType == OptDecimType) params.copy(decimType = DIFDecimType) else params
  val serdes_params  = PacketSerDesParams(params.protoIQ.cloneType, params.numPoints)
  ...
  val inp_if = if (updated_params.decimType == DIFDecimType) {
...
class FFTUnscrambler[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  val io = IO(DIDOIO(params.numPoints, params.protoIQ))
  ...
  (0 until params.numPoints).foreach(i => {
    ...
    io.out.bits(reversed_index) := io.in.bits(index)
  })
}
...
class SDFChainRadix2[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  require(params.decimType != OptDecimType)
  val io = IO(SISOIO(params.protoIQ))
...
class SDFChainRadix4[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T]) extends Module {
  ...
  val delayLog4s        = if (params.decimType == DITDecimType) (0 until numStages) else (0 until numStages).reverse // log2(delay) of each nth stage
...
class SDFStageRadix2[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T], val delay: Int) extends Module {
  ...
  if (params.decimType == DITDecimType) {
    ...
    when (io.cntr > delay.U) {
      inp := io.in * twiddles_rom(io.cntr - delay.U)
    } .otherwise {
      inp := io.in
    }
    io.out := out
    ...
    when (io.cntr < delay.U && io.cntr =/= 0.U) {
      io.out := out * twiddles_rom(io.cntr)
    } .otherwise {
      io.out := out
    }
  }
...
class SDFStageRadix4[T <: Data : Real : BinaryRepresentation](val params: FFTParams[T], val delay: Int) extends Module {
  ...
  if (delay == 1) {
    io.out := out
    inp := io.in
  } else if (params.decimType == DITDecimType) {
    io.out := out
    when (in_counter(cntr_msb, cntr_msb - 1) =/= 0.U) {
      inp := io.in * twiddles_rom(in_counter(cntr_msb, cntr_msb - 1) * in_counter(cntr_msb - 2, 0))
    } .otherwise {
      inp := io.in
    }
  } else {
    inp := io.in
    when (!store_output) {
      io.out := out * twiddles_rom(out_counter(cntr_msb, cntr_msb - 1) * out_counter(cntr_msb - 2, 0))
    } .otherwise {
      io.out := out
    }
  }
