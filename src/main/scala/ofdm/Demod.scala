...
class QPSKDemodUnit[T <: Data : Real](protoIn: DspComplex[T], protoOut: T) extends MultiIOModule {
  val in = Input(protoIn)
  val out = Output(Vec(2, protoOut))
...
class QPSKDemod[T <: Data : Real](params: RXParams[T]) extends MultiIOModule {
  val in = Flipped(Decoupled(Vec(params.nDataSubcarriers, params.protoChannelEst)))
  val out = Decoupled(Vec(params.nDataSubcarriers, Vec(2, params.protoLLR)))
